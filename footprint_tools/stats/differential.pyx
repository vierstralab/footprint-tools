...
cpdef np.ndarray[data_type_t, ndim = 3, mode = 'c'] compute_logpmf_values(dm, np.ndarray[data_type_t, ndim = 2, mode = 'c'] obs, np.ndarray[data_type_t, ndim = 2, mode = 'c'] exp, np.ndarray[data_type_t, ndim = 1, mode = 'c'] bins):
	
	cdef int i 		
	cdef int n = obs.shape[0]
	cdef int m = obs.shape[1]
	cdef int nbins = bins.shape[0]

	cdef np.ndarray[data_type_t, ndim = 3, mode = 'c'] e = np.ascontiguousarray(exp[:,:,np.newaxis] * bins)
	cdef np.ndarray[data_type_t, ndim = 3, mode = 'c'] o = np.ascontiguousarray(np.repeat(obs[:,:,np.newaxis], nbins, axis = 2))
	cdef np.ndarray[data_type_t, ndim = 3, mode = 'c'] res = np.zeros((n, m, nbins), order = 'c')
...
cpdef np.ndarray[data_type_t, ndim = 2, mode = 'c'] compute_log_prior_t(np.ndarray[data_type_t, ndim = 2, mode = 'c'] obs_over_exp, data_type_t [:] bins, data_type_t nu_0, data_type_t sig2_0):
	...
	cdef np.ndarray[data_type_t, ndim = 2, mode = 'c'] res = np.zeros((m, nbins), dtype = np.float64, order = 'c')
	cdef data_type_t [:,:] res_view = res

	for i in range(m):
		
		mu = np.mean(obs_over_exp[:,i])
		ssqdev = np.sum((obs_over_exp[:,i]-mu)**2)
		...
		with nogil:
			for j in range(nbins):
				...
				res_view[i, j] = t_logpmf(bins[j], nu_1, mu, sig2_1)

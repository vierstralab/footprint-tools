...
@optgroup.option(
    "--outfile", type=click.STRING, default="beta.txt", help="Output file path"
)
def run(bedgraph_file, fdr_cutoff=0.05, exp_cutoff=10, outfile="beta.txt"):
    ...
    logger.info(
        f"Filtering nucleotides with expected count >= {exp_cutoff} and within a FDR {fdr_cutoff} footprint"
    )
        ...
        try:
            ...
            for line in filehandle:
                ...
                if total_lines % 500000 == 0:
                    sp.text = (
                        "Reading nucleotides -- {:,} / {:,} passed filters".format(
                            total_passed, total_lines
                        )
                    )
    ...
    logger.info(
        f"Beta distribution parameters: \u03b1 = {prior[0]}, \u03b2 = {prior[1]}"
    )

...
class deviation_stats(dataset):
    ...
    def __init__(self, interval_file, bam_file, fasta_file, bm, dm, **kwargs):
        ...
        self.counts_predictor_kwargs = get_kwargs(
            ["half_win_width", "smoothing_half_win_width", "smoothing_clip"], kwargs
        )
        ...
        if self.dm:
            try:
                ...
                stats = np.column_stack(
                    (exp, obs, -np.log(pvals), -np.log(win_pvals), efdr)
                )
        else:
            stats = np.column_stack((exp, obs))

        return {"interval": interval, "stats": stats}
...
def run(
    ...
    try:
        ...
        write_output_header(
            ["exp", "obs", "-log(pval)", "-log(winpval)", "fdr"],
            file=output_bedgraph_filehandle,
            include_name=False,
            extra=args,
        )
        ...
        if len(write_footprints) > 0:
            logger.info(
                f"Writing FDR thresholded footprints to {output_bed_file_template.format('{threshold}')} for threshold \u22f2 {write_footprints}"
            )
            for t in write_footprints:
                ...
                write_output_header(
                    ...
                    file=fh,
                    include_name=True,
                    extra=[args, f"thresholded @ FDR {t}"],
                )
        ...
        for batch in tqdm(dl_iter, colour="#cc951d"):
            for interval, stats in zip(batch["interval"], batch["stats"]):
                ...
                for thresh, fh in output_bed_filehandles.items():
                    ...
                    write_segments_to_output(
                        interval, stats[:, -1], thresh, file=fh, decreasing=True
                    )

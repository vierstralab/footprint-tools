    ...
    def _parse_tuple(ctx, params, value):
        ...
        try:
            ...
            raise click.BadOptionUsage(
                f"needs to be a comma-delimited tuple of type {value_type.__name__}"
            )
        return items
...
def list_args(value_type=int):
    def _parse_list(ctx, params, value):
        ...
        try:
            ...
            raise click.BadOptionUsage(
                f"needs to be a comma-delimited list of type {value_type.__name__}"
            )
        return items
...
def verify_bam_file(fn, is_cram=False):
...
def write_output_header(
    columns, file=sys.stdout, delim="\t", include_name=True, extra=None
):
    ...
    include_name : bool
        Include name column before writing additional columns. Makes
        the file consistent with BED3+2 format
    """
    file.write(
        f"# generated by {footprint_tools.__name__} version {footprint_tools.__version__}\n"
    )
    if extra:
...
def write_stats_to_output(
    interval, stats, file=sys.stdout, delim="\t", filter_fn=None, fmt_string="0.4f"
):
    ...
    for i in idxs:
        ...
        out += delim.join(
            [("{0:" + fmt_string + "}").format(val) for val in stats[i, :]]
        )
        file.write(out + "\n")


def write_segments_to_output(
    interval,
    stats,
    threshold,
    name=".",

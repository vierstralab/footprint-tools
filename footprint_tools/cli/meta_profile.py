...
def run(interval_file, bam_file, n_threads=8):
    ...
    dp = profile_loader(interval_file, bam_file)
    dp_iter = ds.batch_iter(batch_size=100, collate_fn=list_collate, num_workers=n_threads)

    with logging_redirect_tqdm():
        for x in tqdm(dp_iter, colour='#C70039'):

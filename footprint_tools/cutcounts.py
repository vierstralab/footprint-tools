...
class bamfile(object):
	...
	def __init__(self, filepath, min_qual = 1, remove_dups = False, remove_qcfail = True, chunksize = 1000, offset = (0, 0), use_cache = False):
		...
		for alignedread in self.samfile.fetch(chrom, max(start, 0), end):
			...
			try:
				...
				if alignedread.is_reverse:
					a = int(alignedread.reference_end) - 1
					if a < end:	
	...
	def __lookup_from_cache(self, chrom, start, end):
		...
		fw_cutarray  = np.array([self.cache[chrom]["+"].get(i, 0.0) for i in range(start + self.offset[0], end + self.offset[0])])
		rev_cutarray = np.array([self.cache[chrom]["-"].get(i, 0.0) for i in range(start + self.offset[1], end + self.offset[1])])
	...
	def __lookup(self, chrom, start, end):
		...
		for alignedread in self.samfile.fetch(chrom, max(start, 0), end):
			...
			try:
				...
				if alignedread.is_reverse:
					...
					a = int(alignedread.reference_start) # pysam is zero-based!
					if a >= start:
		...
		fw_cutarray = np.array([tmp_fw.get(i, 0.0) for i in range(start + self.offset[0], end + self.offset[0])])
		rev_cutarray = np.array([tmp_rev.get(i, 0.0) for i in range(start + self.offset[1], end + self.offset[1])])

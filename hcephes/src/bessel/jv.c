...
static double hcephes_hankel(double n, double x);
static double hcephes_jnt(double n, double x);
static double hcephes_jnx(double n, double x);
...
HCEPHES_API double hcephes_jv(double n, double x) {
    ...
    if ((x < 0.0) && (y != an)) {
        hcephes_mtherr("Jv", HCEPHES_DOMAIN);
        y = 0.0;
        goto done;
    }
    ...
    t = 3.6 * sqrt(an);
    if ((y < t) && (an > 21.0))
        return (sign * hcephes_jvs(n, x));
    if ((an < k) && (y > 21.0))
        return (sign * hcephes_hankel(n, x));
    ...
    if (an < 500.0) {
         ...
         */
        if (n < 0.0) {
            hcephes_mtherr("Jv", HCEPHES_TLOSS);
            y = 0.0;
            goto done;
        }
        t = x / n;
...
fstart:
    ...
    do {
        ...
        if (++ctr > 1000) {
            hcephes_mtherr("jv", HCEPHES_UNDERFLOW);
            goto done;
        }
        if (t < HCEPHES_MACHEP)
            goto done;
...
static double hcephes_jvs(double n, double x) {
    ...
    if ((ex > -1023) && (ex < 1023) && (n > 0.0) && (n < (MAXGAM - 1.0))) {
        ...
        t = n * log(0.5 * x) - hcephes_lgam(n + 1.0);
        if (y < 0) {
            sgngam = -sgngam;
            y = -y;
        }
        ...
        if (t > HCEPHES_MAXLOG) {
            hcephes_mtherr("Jv", HCEPHES_OVERFLOW);
            return (HUGE_VAL);
        }
        y = sgngam * exp(t);
    }
...
hank1:
    u = x - (0.5 * n + 0.25) * HCEPHES_PI;
    t = sqrt(2.0 / (HCEPHES_PI * x)) * (pp * cos(u) - qq * sin(u));
...
static double P1[] = {-2.083333333333333333333333E-1, 1.250000000000000000000000E-1};
static double P2[] = {3.342013888888888888888889E-1, -4.010416666666666666666667E-1,
                      7.031250000000000000000000E-2};
static double P3[] = {-1.025812596450617283950617E+0, 1.846462673611111111111111E+0,
                      -8.912109375000000000000000E-1, 7.324218750000000000000000E-2};
static double P4[] = {4.669584423426247427983539E+0, -1.120700261622299382716049E+1,
...
static double hcephes_jnx(double n, double x) {
    ...
    u[0] = 1.0;
    zzi = 1.0 / zz;
    u[1] = hcephes_polevl(zzi, P1, 1) / sz;
    u[2] = hcephes_polevl(zzi, P2, 2) / zz;
    u[3] = hcephes_polevl(zzi, P3, 3) / (sz * zz);
    pp = zz * zz;
    u[4] = hcephes_polevl(zzi, P4, 4) / pp;
    u[5] = hcephes_polevl(zzi, P5, 5) / (pp * sz);
    pp *= zz;
    u[6] = hcephes_polevl(zzi, P6, 6) / pp;
    u[7] = hcephes_polevl(zzi, P7, 7) / (pp * sz);
...
static double hcephes_jnt(double n, double x) {
    ...
    F[0] = 1.0;
    F[1] = -z / 5.0;
    F[2] = hcephes_polevl(z3, PF2, 1) * zz;
    F[3] = hcephes_polevl(z3, PF3, 2);
    F[4] = hcephes_polevl(z3, PF4, 3) * z;
    G[0] = 0.3 * zz;
    G[1] = hcephes_polevl(z3, PG1, 1);
    G[2] = hcephes_polevl(z3, PG2, 2) * z;
    G[3] = hcephes_polevl(z3, PG3, 2) * zz;

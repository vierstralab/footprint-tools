...
static double A[6] = {
    -5.350447357812542947283E0, 2.185049168816613393830E2,  -4.176572384826693777058E3,
    5.541176756393557601232E4,  -3.313381331178144034309E5, 1.592627163384945414220E6,
};
static double B[6] = {
    /*  1.000000000000000000000E0, */
    -5.250547959112862969197E1, 1.259616186786790571525E3,  -1.756549581973534652631E4,
...
static double B5[8] = {
    /* 1.000000000000000000000E0, */
    8.585231423622028380768E-1,  4.483285822873995129957E-1, 7.687932158124475434091E-2,
...
static double A6[8] = {
    1.981808503259689673238E-2,  -1.271645625984917501326E0,  -2.088160335681228318920E0,
...
static double B6[7] = {
    /* 1.000000000000000000000E0, */
    1.476498670914921440652E0,  5.629177174822436244827E-1, 1.699017897879307263248E-1,
...
static double B7[5] = {
    /* 1.000000000000000000000E0, */
    -1.002252150365854016662E0, 2.928709694872224144953E-1,  -3.337004338674007801307E-2,
...
HCEPHES_API double hcephes_ei(double x) {
    ...
    if (x <= 0.0) {
        hcephes_mtherr("ei", HCEPHES_DOMAIN);
        return 0.0;
    } else if (x < 2.0) {
        /* Power series.
                                  inf    n
           ...
           Ei(x) = EUL + ln x  +   >   ----
                                  ...
                                  n=1
        */
        f = hcephes_polevl(x, A, 5) / hcephes_p1evl(x, B, 6);
        /*      f = hcephes_polevl(x,A,6) / hcephes_p1evl(x,B,7); */
        /*      f = hcephes_polevl(x,A,8) / hcephes_p1evl(x,B,9); */
        ...
        */
        w = 1.0 / x;
        f = hcephes_polevl(w, A6, 7) / hcephes_p1evl(w, B6, 7);
        return (exp(x) * w * (1.0 + w * f));
    } else if (x < 8.0) {
        w = 1.0 / x;
        f = hcephes_polevl(w, A5, 7) / hcephes_p1evl(w, B5, 8);
        return (exp(x) * w * (1.0 + w * f));
    } else if (x < 16.0) {
        w = 1.0 / x;
        f = hcephes_polevl(w, A2, 9) / hcephes_p1evl(w, B2, 9);
        return (exp(x) * w * (1.0 + w * f));
    } else if (x < 32.0) {
        w = 1.0 / x;
        f = hcephes_polevl(w, A4, 7) / hcephes_p1evl(w, B4, 8);
        return (exp(x) * w * (1.0 + w * f));
    } else if (x < 64.0) {
        w = 1.0 / x;
        f = hcephes_polevl(w, A7, 5) / hcephes_p1evl(w, B7, 5);
        return (exp(x) * w * (1.0 + w * f));
    } else {
        w = 1.0 / x;
        f = hcephes_polevl(w, A3, 8) / hcephes_p1evl(w, B3, 9);
        return (exp(x) * w * (1.0 + w * f));
    }
}
